...
from odoo import http, _, tools, fields, SUPERUSER_ID
...
class CustomRentalSale(http.Controller):
    ...
    #            check_in_date = datetime.strptime(localdatetime, '%m/%d/%Y %I:%M %p')
                check_in_date = parse(localdatetime)
        except:
            if request.env.lang in ['zh_CN', 'zh_HK', 'zh_TW']:
                lc_date = localdatetime
                for i in ['凌', '晨', '点', '分']:
                    if i == '点':
                        lc_date = lc_date.replace(i, ':')
                    else:
                        lc_date = lc_date.replace(i, '')
                if '下午' in lc_date or '晚上' in lc_date or '中午' in lc_date:
                    lc_date = lc_date.replace('下午', '')
                    lc_date = lc_date.replace('晚上', '')
                    lc_date = lc_date.replace('中午', '')
                    lc_date = lc_date + 'PM'
                elif '凌晨' in lc_date or '上午' in lc_date or '早上' in lc_date:
                    lc_date = lc_date.replace('凌晨', '')
                    lc_date = lc_date.replace('上午', '')
                    lc_date = lc_date.replace('早上', '')
                    lc_date = lc_date + 'AM'
                localdatetime = lc_date
        ...
        if user_id.tz:
            ...
            timezone_tz = request.env['res.users'].sudo().browse(SUPERUSER_ID).tz or 'utc'
    ...
    def create_custom_rental_so(self, **kw):
        ...
        if kw.get("rental_sale_order"):
            ...
            company_x = request.website.company_id.id # 17/03/2021
            sale_order_id = sale_order_obj.with_context(company_id=company_x).with_company(company_x).sudo().create(values) # 17/03/2021
            # sale_order_id = sale_order_obj.with_context(force_company=request.website.company_id.id).sudo().create(values) # 17/03/2021
